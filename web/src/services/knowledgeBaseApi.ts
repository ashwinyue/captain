...
export const KNOWLEDGE_BASE_ENDPOINTS = {
  ...
  QA_PAIRS_STATS: (collectionId: string) => `/${API_VERSION}/rag/${collectionId}/qa-pairs/stats`,
...
export class KnowledgeBaseApiService extends BaseApiService {
  ...
  /**
   * Get QA pair statistics for a collection
   * GET /v1/rag/{collection_id}/qa-pairs/stats
   */
  static async getQAStats(collectionId: string): Promise<QAStatsResponse> {
    const service = new KnowledgeBaseApiService();
    try {
      return await service.get<QAStatsResponse>(
        KNOWLEDGE_BASE_ENDPOINTS.QA_PAIRS_STATS(collectionId)
      );
    } catch (error) {
      throw new Error(service['handleApiError'](error));
    }
