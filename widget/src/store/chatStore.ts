...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  sendMessage: async (text: string) => {
    ...
    try {
      ...
      // 处理转人工成功事件 - 显示系统消息
      if (resJson.event_type === 'transfer_success' && resJson.content) {
        const transferMsg: ChatMessage = {
          id: 'sys-transfer-' + Date.now(),
          role: 'agent',
          payload: { type: 1, content: resJson.content },
          time: new Date(),
        }
        set(state => ({ messages: [...state.messages, transferMsg] }))
        // 更新用户消息状态为已发送
        set(state => ({
          messages: state.messages.map(m => m.id === id ? { ...m, status: undefined } : m)
        }))
        return // 转人工后不需要继续发送 WebSocket 消息
      }

      // 处理已在人工客服中事件 - 显示提示消息
      if (resJson.event_type === 'already_transferred' && resJson.content) {
        const alreadyTransferredMsg: ChatMessage = {
          id: 'sys-already-transferred-' + Date.now(),
          role: 'agent',
          payload: { type: 1, content: resJson.content },
          time: new Date(),
        }
        set(state => ({ messages: [...state.messages, alreadyTransferredMsg] }))
        // 更新用户消息状态为已发送
        set(state => ({
          messages: state.messages.map(m => m.id === id ? { ...m, status: undefined } : m)
        }))
        // 继续执行 WebSocket 发送，让人工客服收到消息
      }

      // 处理已转人工后的消息 - 需要 Widget 通过 WebSocket 发送给客服
      // event_type: message_sent_human 表示后端不发送，由 Widget 发送
      // 继续执行下面的 WebSocket 发送逻辑
